...
async function isEnabledNextButton(button) {
...
export async function navigatePaginatedAdOverview(controller, pageUrl, pageAction, { maxPages = 10, paginationFollowUpTimeout, paginationInitialTimeout, } = {}) {
    try {
    ...
    let multiPage = false;
    try {
        const nextButtons = await controller.webFindAll(By.CSS_SELECTOR, nextPageSelector, { timeout: paginationInitialTimeout });
    ...
    while (currentPage <= maxPages) {
        ...
        try {
            const nextButtons = await controller.webFindAll(By.CSS_SELECTOR, nextPageSelector, { timeout: paginationFollowUpTimeout });
            let nextButton = null;
...
export async function extractOwnAdUrls(controller, rootUrl, { paginationFollowUpTimeout, paginationInitialTimeout, } = {}) {
    const refs = [];
    await navigatePaginatedAdOverview(controller, `${rootUrl}/m-meine-anzeigen.html`, async () => {
        try {
            const adListContainer = await controller.webFind(By.ID, "my-manageitems-adlist");
            const listItems = await controller.webFindAll(By.CLASS_NAME, "cardbox", { parent: adListContainer });
            const pageRefs = [];
            for (const listItem of listItems) {
                try {
                    const link = await controller.webFind(By.CSS_SELECTOR, "div h3 a.text-onSurface", { parent: listItem });
