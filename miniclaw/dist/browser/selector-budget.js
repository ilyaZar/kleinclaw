...
export function allocateSelectorGroupBudgets(totalTimeout, selectorCount) {
    if (selectorCount <= 0) {
    ...
    const reserveForBackups = BACKUP_SELECTOR_BUDGET_FLOOR_SECONDS * (selectorCount - 1);
    let primary = Math.min(totalTimeout * PRIMARY_SELECTOR_BUDGET_RATIO, totalTimeout - reserveForBackups);
    ...
    for (let index = 0; index < selectorCount - 1; index += 1) {
        ...
        const minReserve = BACKUP_SELECTOR_BUDGET_FLOOR_SECONDS * remainingSlotsAfterThis;
