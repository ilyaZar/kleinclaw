...
export class ChromeVersionInfo {
    ...
    majorVersion;
    browserName;
    constructor(versionString, majorVersion, browserName = "Unknown") {
...
export function normalizeBrowserName(browserName) {
...
export async function detectChromeVersionFromBinary(binaryPath, { timeout = 10 } = {}) {
    try {
        ...
        return new ChromeVersionInfo(versionString, majorVersion, normalizeBrowserName(binaryPath));
    }
    catch {
        return null;
    }
}
export async function detectChromeVersionFromRemoteDebugging({ host = "127.0.0.1", port = 9222, timeout = 5, } = {}) {
    try {
        ...
        return new ChromeVersionInfo(match[1], parseVersionString(match[1]), normalizeBrowserName(browser));
    }
    catch {
        return null;
    }
}
export function validateChrome136Configuration(browserArguments, userDataDir) {
    const hasUserDataDirArg = browserArguments.some((arg) => arg.startsWith("--user-data-dir="));
...
function accessOk(filePath, mode) {
    try {
...
export function buildBrowserDiagnosticReport(config) {
    const lines = [];
    ...
    if (remoteDebuggingPort === null) {
        ...
        const [chrome136Valid, chrome136Message] = validateChrome136Configuration(browserConfig.arguments, browserConfig.userDataDir);
        if (chrome136Valid) {
            lines.push({
                status: "ok",
...
export function chromeVersionProbeOptionsFromConfig(config) {
    const browserConfig = config.browser;
    const binaryPath = browserConfig.binaryLocation.trim();
    return {
...
export async function getChromeVersionDiagnosticInfo({ binaryPath, remoteHost = "127.0.0.1", remotePort, remoteTimeout, binaryTimeout, } = {}) {
    ...
    if (diagnosticInfo.chrome_136_plus_detected) {
        diagnosticInfo.recommendations.push("Chrome 136+ detected - ensure --user-data-dir is configured for remote debugging");
    }
    return diagnosticInfo;
}
export async function getChromeVersionDiagnosticInfoFromConfig(config) {
    return getChromeVersionDiagnosticInfo(chromeVersionProbeOptionsFromConfig(config));
}
