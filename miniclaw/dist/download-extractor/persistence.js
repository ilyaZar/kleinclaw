...
export const AD_SCHEMA_HEADER = "# yaml-language-server: $schema=miniclaw://schemas/ad.schema.json";
...
export async function removeTreeWithRetries(target) {
...
export async function saveDownloadedAd({ adConfig, adFileStem, adId, finalDir, stagingDir, }) {
    const adFilePath = path.join(stagingDir, `${adFileStem}.yaml`);
    const backupDir = path.join(path.dirname(finalDir), `${BACKUP_DIR_PREFIX}${adFileStem}`);
    let backupCreatedByUs = false;
    try {
        await saveDataFile(adFilePath, adConfig, { header: AD_SCHEMA_HEADER });
        if (await fileExists(backupDir)) {
            throw new FileExistsError(`Backup directory ${backupDir} already exists. ` +
