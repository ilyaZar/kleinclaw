...
import { CITY_LISTBOX_ID_FALLBACK, CITY_SELECTED_OPTION_ID, } from "./constants.js";
import { setFrameworkInputValue } from "./core-fields.js";
import { clickElement, elementAttribute, elementInputValue, elementLocalName, inspectSpecialAttributeElement, visibleElementText, } from "./element-helpers.js";
function normalizeLocationText(value) {
    return value.trim().replace(/\s+/g, " ").toLowerCase();
}
export function locationMatchesTarget(target, candidate) {
...
export function cityListboxOptionSelector(listboxId) {
...
async function cityOptionText(element) {
    return (await visibleElementText(element)).replace(/\s+/g, " ").trim();
}
export async function readCitySelectionText(controller, { quickDomTimeout } = {}) {
    let cityElement;
    try {
        cityElement = await controller.webFind(By.ID, "ad-city", { timeout: quickDomTimeout });
    }
    catch (error) {
    ...
    try {
        const selectedOption = await controller.webFind(By.ID, CITY_SELECTED_OPTION_ID, { timeout: quickDomTimeout });
...
export async function selectCityComboboxOption(controller, target, { quickDomTimeout } = {}) {
    await controller.webClick(By.ID, "ad-city", quickDomTimeout);
    const cityElement = await controller.webFind(By.ID, "ad-city", { timeout: quickDomTimeout });
    ...
    const options = await controller.webFindAll(By.CSS_SELECTOR, cityListboxOptionSelector(listboxId), { timeout: quickDomTimeout });
    const normalizedTarget = normalizeLocationText(target);
    const exactMatches = [];
    const citySuffixMatches = [];
    const prefixMatches = [];
    ...
    let selected = exactMatches[0] ?? null;
    if (selected === null && citySuffixMatches.length > 1) {
        throw new TimeoutError(`Ambiguous contact location '${target}' matched multiple city options`);
    }
...
export async function setContactLocation(controller, location, { quickDomTimeout } = {}) {
    ...
    if (info.localName !== "button" || info.role !== "combobox") {
        throw new TimeoutError(`Unsupported city element type while setting contact location: ${info.localName}`);
    }
    await selectCityComboboxOption(controller, target, { quickDomTimeout });
}
export async function setContactFields(controller, contact, { quickDomTimeout } = {}) {
    if (contact.zipcode) {
    ...
    if (contact.phone) {
        try {
            const phoneElement = await controller.webProbe(By.ID, "ad-phone", { timeout: quickDomTimeout });
