...
import { CATEGORY_CHANGE_CONTROL_XPATH, CATEGORY_NEXT_BUTTON_XPATH, CATEGORY_PICKER_RADIO_SELECTOR, } from "./constants.js";
import { CategoryResolutionError } from "./errors.js";
import { clickElement, elementAttribute, stringOrNull, visibleElementText, xpathLiteral, } from "./element-helpers.js";
export async function resolveCategorySuggestions(controller, category, { quickDomTimeout } = {}) {
    ...
    let radioByValue = new Map();
    for (let attempt = 0; attempt < 2; attempt += 1) {
        let radios = [];
        try {
            radios = await controller.webFindAll(By.CSS_SELECTOR, CATEGORY_PICKER_RADIO_SELECTOR, { timeout: quickDomTimeout });
        }
        catch (error) {
    ...
    if (radioByValue.size === 0) {
        throw new TimeoutError("Category suggestion picker element found but no radio suggestions " +
    ...
    for (const segment of segments) {
        ...
        try {
            if (radioId) {
                await controller.webClick(By.XPATH, "//fieldset[@id='ad-category-picker']" +
                    `//label[@for=${xpathLiteral(radioId)}]`, quickDomTimeout);
            }
            else {
    ...
    throw new CategoryResolutionError("Category suggestion picker shown, but no segment of configured path " +
        `'${category}' matched the offered suggestions [${offered}]. Update the ` +
        "ad's 'category' to an offered ID or a valid full path.");
}
export async function setCategory(controller, { adFile, category, rootUrl, quickDomTimeout }) {
    ...
    if (!category) {
        if (!isCategoryAutoSelected) {
            throw new Error(`No category specified in [${adFile}] and automatic category detection failed`);
        }
        return;
    }
