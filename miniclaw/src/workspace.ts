...
interface FootprintResult {
  mode: InstallationMode | "ambiguous" | "unknown";
  portableHits: string[];
  xdgHits: string[];
}

function runtimeCwd(options: WorkspaceResolutionOptions = {}): string {
  return path.resolve(options.cwd ?? process.cwd());
}

function runtimeEnv(options: WorkspaceResolutionOptions = {}): NodeJS.ProcessEnv {
  return options.env ?? process.env;
}

function runtimeHome(options: WorkspaceResolutionOptions = {}): string {
  return path.resolve(options.homeDir ?? os.homedir());
}

function resolveFromCwd(value: string, options: WorkspaceResolutionOptions = {}): string {
  return path.resolve(runtimeCwd(options), value);
}

function isRelativeTo(candidate: string, parent: string): boolean {
...
export function workspaceForConfig(
  configFile: string,
...
function buildXdgWorkspace(
  logBasename: string,
  configFileOverride: string | null,
  options: WorkspaceResolutionOptions = {},
): Workspace {
...
function detectModeFromFootprintsWithHits(
  configFile: string,
  options: WorkspaceResolutionOptions = {},
): FootprintResult {
  const resolvedConfig = path.resolve(configFile);
  ...
  const portableHits: string[] = [];
  const xdgHits: string[] = [];
...
function formatHits(label: string, hits: string[]): string {
  if (!hits.length) {
    return `${label}: none`;
  }
  return `${label}:\n- ${dedupe(hits).join("\n- ")}`;
...
function workspaceModeResolutionError(
  configFile: string,
  detectedMode: "ambiguous" | "unknown",
  portableHits: string[],
  xdgHits: string[],
): Error {
  const guidance =
    `Cannot determine workspace mode for --config=${configFile}. ` +
...
function promptInstallationMode(options: WorkspaceResolutionOptions = {}): InstallationMode {
...
export function resolveWorkspace(args: ResolveWorkspaceArgs): Workspace {
  ...
  if (configPath && mode === null) {
    ...
    if (detected.mode === "portable" || detected.mode === "xdg") {
      ...
      throw workspaceModeResolutionError(
        configPath,
        detected.mode,
        detected.portableHits,
        detected.xdgHits,
      );
    }
  }
