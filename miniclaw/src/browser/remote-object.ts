...
export function isRemoteObjectLike(value: unknown): boolean {
  if (!isRecord(value)) {
    return false;
  }
...
export function convertRemoteObjectValue(data: unknown): unknown {
  if (Array.isArray(data)) {
    if (
      ...
      for (const item of data) {
        ...
        converted[String(key)] = convertRemoteObjectValue(
...
export function normalizeRemoteObjectResult(result: unknown): unknown {
  if (!isRemoteObjectLike(result)) {
    return result;
  }

  try {
    const remoteObject = result as {
      value?: unknown;
      deep_serialized_value?: { value?: unknown } | null;
      deepSerializedValue?: { value?: unknown } | null;
    };
    ...
    const deepValue =
      remoteObject.deep_serialized_value?.value ??
