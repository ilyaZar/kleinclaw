...
export function normalizeSpecialAttributeKey(key: string): string {
  return key.replace(/_[a-z]+$/, "").split(".").at(-1) ?? "";
}

export function conditionCandidateValues(conditionValue: string): string[] {
...
export function specialAttributeXPath(key: string): string {
...
export function specialAttributeCandidatePriority(
  info: Pick<SpecialAttributeElementInfo, "localName" | "role" | "type">,
): [number, number] {
  if (info.localName === "button" && info.role === "combobox") {
    return [0, 0];
  }
  if (
    info.localName === "input" &&
  ...
  if (
    (info.localName === "input" || info.localName === "textarea") &&
    info.type !== "hidden"
  ) {
    return [4, 0];
  }
...
export async function pickSpecialAttributeCandidate(
  candidates: WebElement[],
  specialAttributeKey: string,
): Promise<{ element: WebElement; info: SpecialAttributeElementInfo }> {
  if (candidates.length === 0) {
    throw new TimeoutError(
      `No candidates found for special attribute [${specialAttributeKey}]`,
    );
  }
...
function desiredCheckboxState(key: string, value: string): boolean {
...
function currentCheckboxState(checked: unknown): boolean {
...
export async function setConditionDialog(
  controller: Pick<
    PublishingFormController,
    "webClick" | "webFind" | "webProbe"
  >,
  conditionValue: string,
  { quickDomTimeout }: PublishingFormOptions = {},
): Promise<boolean> {
  const conditionTrigger = await controller.webProbe(
    By.XPATH,
    CONDITION_TRIGGER_XPATH,
  ...
  const triggerControls = String(
    await elementAttribute(conditionTrigger, "aria-controls") ?? "",
  );
  if (
    triggerId.toLowerCase().includes("shipping") ||
  ...
  try {
    await clickElement(
      conditionTrigger,
      "Failed to set attribute 'condition_s'",
    );
    await controller.webFind(
      By.XPATH,
      '//*[self::dialog or @role="dialog"]',
      { timeout: quickDomTimeout },
    );

    let conditionRadio: WebLocator | null = null;
    for (const candidate of conditionCandidateValues(conditionValue)) {
      conditionRadio = await controller.webProbe(
        By.XPATH,
        "//*[self::dialog or @role='dialog']//input" +
        `[@type='radio' and @value=${xpathLiteral(candidate)}]`,
        { timeout: quickDomTimeout },
      );
    ...
    const conditionRadioId = stringOrNull(
      await elementAttribute(conditionRadio, "id"),
    );
    if (conditionRadioId) {
      try {
        await controller.webClick(
          By.XPATH,
          "//*[self::dialog or @role='dialog']" +
          `//label[@for=${xpathLiteral(conditionRadioId)}]`,
          quickDomTimeout,
        );
      } catch (error) {
  ...
  try {
    await controller.webClick(
      By.XPATH,
      '//*[self::dialog or @role="dialog"]//button[.//span[text()="Bestätigen"]]',
      quickDomTimeout,
    );
  } catch (error) {
    if (error instanceof TimeoutError) {
...
export async function setSpecialAttributes(
  controller: Pick<
    PublishingFormController,
    | "webClick"
    | "webExecute"
  ...
  { setCondition }: SetSpecialAttributesOptions = {},
): Promise<void> {
  ...
  for (const [key, rawValue] of Object.entries(specialAttributes)) {
    ...
    if (normalizedKey === "condition") {
      const conditionHandler = setCondition ??
        ((conditionValue: string) =>
          setConditionDialog(controller, conditionValue));
    ...
    let selected: { element: WebElement; info: SpecialAttributeElementInfo };
    try {
    ...
    try {
      if (selected.info.localName === "select") {
        ...
        selected.info.localName === "button" &&
