...
export async function imageMarkerValue(marker: WebElement): Promise<string> {
  const value = await elementAttribute(marker, "value");
  return String(value ?? "").trim();
}

async function countImageMarkers(
  controller: Pick<PublishingFormController, "webFindAll">,
  quickDomTimeout?: number,
): Promise<number> {
  const markers = await controller.webFindAll(
    By.CSS_SELECTOR,
    IMAGE_MARKER_SELECTOR,
...
async function pollImageUploadCondition(
  controller: Pick<PublishingFormController, "webSleep">,
  condition: () => Promise<boolean>,
  imageUploadTimeout = 30,
): Promise<void> {
...
export async function uploadImages(
  controller: Pick<
    ...
    "webFind" | "webFindAll" | "webSleep"
  >,
  ad: PublishingImagesAd,
  {
    imageUploadTimeout = 30,
    quickDomTimeout,
    waitForImageUpload,
  ...
  for (const image of ad.images) {
    const fileInput = await controller.webFind(
      By.CSS_SELECTOR,
      IMAGE_FILE_INPUT_SELECTOR,
    );
  ...
  const checkImagesUploaded = async () =>
    (await countProcessedImages()) >= expectedCount;
  ...
  try {
    if (waitForImageUpload) {
      ...
      await pollImageUploadCondition(
        controller,
        checkImagesUploaded,
        imageUploadTimeout,
      );
    ...
    throw new TimeoutError(
      "Not all images were uploaded within timeout. " +
      `Expected ${expectedCount}, found ${currentCount} processed images.`,
    );
  }
}

export async function cleanupExistingImages(
  controller: Pick<
    PublishingFormController,
    "webFindAll" | "webProbe" | "webSleep"
  >,
  ...
  for (let index = 0; index < existingImageCount; index += 1) {
    const removeButton = await controller.webProbe(
      By.CSS_SELECTOR,
      IMAGE_REMOVE_BUTTON_SELECTOR,
      { timeout: quickDomTimeout },
    );
    if (removeButton === null) {
      throw new TimeoutError(
        "Image cleanup failed before upload. " +
        `Removed ${index} of ${existingImageCount} existing images.`,
      );
    }
    await clickElement(
      removeButton,
      "Image cleanup remove button cannot be clicked",
    );
    removedCount += 1;
