...
import { type CommandPlan, type ParsedArgs, type PlannedAd } from "./types.js";

function planAd(ad: LoadedAd): PlannedAd {
  return {
...
export async function planCommand(
  ...
  const excludeAdsWithId =
    parsed.command === "verify" || parsed.command === "update-content-hash"
      ? false
      : true;
  ...
  const needsBrowser = ["publish", "update", "delete", "extend"].includes(
    parsed.command,
  ) && ads.length > 0;
