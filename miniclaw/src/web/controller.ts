...
function firstAvailableDescription(
...
export class WebController {
  ...
  async webFindFirstAvailable(
    ...
    return this.runWithTimeoutRetries(
      (effectiveTimeout) => this.webFindFirstAvailableWithinBudget(
        selectors,
        effectiveTimeout,
        parent,
      ),
  ...
  async webSelect(
    ...
    await this.webAwait(
      () => this.webCheck(type, value, Is.CLICKABLE),
      {
        timeout,
        timeoutErrorMessage:
  ...
  async webOpen(
    ...
    await this.webAwait(
      () => this.webExecute("document.readyState == 'complete'"),
      {
        applyMultiplier: false,
        timeout: effectiveTimeout,
        timeoutErrorMessage:
          `Page did not finish loading within ${effectiveTimeout} seconds.`,
      },
    );
  }
  ...
  private async isSelected(element: WebElement): Promise<boolean> {
    if (element.evaluate) {
      return Boolean(await element.evaluate(`
        function (element) {
